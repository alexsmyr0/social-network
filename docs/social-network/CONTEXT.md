...
- Planning now covers six delivery phases. Only Phase 1 (foundations and account access) is ticketed: 17 unstarted tickets (8 A, 9 B) after scope/dependency review. Start SN-A01 and SN-B01; decision order is SN-A02 → SN-B02 → SN-B08. No architecture choice was made by the audit.
- Remote `main` contains the clean import snapshot `b295348`; local `main` now follows it. Earlier push rejection concerned legacy history on the old branch. Keep new work on the clean history; hosted CI evidence remains a future ticket gate.
